...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void YoloV8_ncnn::init(AAssetManager *mgr,const char* path_param, const  char* path_bin, int imgsz, bool gpu)
{
//    printf_s("init model -------------------------------- \n");
...
//    printf_s("imgsz      :%d \n", imgsz);
    ...
    //
    model.load_param(mgr,path_param);
    model.load_model(mgr,path_bin);
}
void YoloV8_ncnn::detect(const cv::Mat& rgb, std::vector<Object>& objects, float threshold_confidence, float threshold_nms)
{
    objects.clear();
    ncnn::Mat mat_in;
    ncnn::Mat mat_out;
...
void YoloV8_ncnn::NMSBoxes_custom(const std::vector<cv::Rect>& _boxes, std::vector<int>& _indexes_nms, float _nms_threshold)
{
    _indexes_nms.clear();
...
float YoloV8_ncnn::imageProcess(const cv::Mat& image_in, ncnn::Mat& image_out, int imgsz) {
...
float YoloV8_ncnn::getValueAtPosition(const ncnn::Mat& mat, int col, int row) {
    return mat[row * mat.w + col];
}
void YoloV8_ncnn::generate_proposals(const ncnn::Mat& mat, float scale, float threshold_confidence, std::vector<cv::Rect>& boxes, std::vector<float>& confidences, std::vector<int>& class_ids) {
    boxes.clear();
    confidences.clear();
    class_ids.clear();
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void OCR::init(AAssetManager *mgr,const char* path_param, const  char* path_bin, int imgsz, bool gpu) {
    ocr = new YoloV8_ncnn();
    ocr->init(mgr,path_param, path_bin, imgsz, gpu);
}
void OCR::read(const cv::Mat& rgb, std::string& text) {
    std::vector<Object> objects;
    ocr->detect(rgb, objects, 0.5f, 0.2f);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void GARIAMAN::init(AAssetManager *mgr,bool use_gpu) {
    __android_log_print(ANDROID_LOG_DEBUG, "gariaman", "gpu : %d" ,use_gpu);

    if (this->p_detector != 0){
        __android_log_print(ANDROID_LOG_DEBUG, "gariaman", "delete p_detector");
        delete this->p_detector;
        this->p_detector = 0;
    }
    if (this->p_ocr != 0){
        __android_log_print(ANDROID_LOG_DEBUG, "gariaman", "delete p_ocr");
        delete this->p_ocr;
        this->p_ocr = 0;
    }

    if (this->p_detector == 0){
        __android_log_print(ANDROID_LOG_DEBUG, "gariaman", "loadModel p_detector");
        this->p_detector = new YoloV8_ncnn();
        this->p_detector->init(mgr,"_car+plate_320.param", "_car+plate_320.bin", 320, use_gpu);
    }

    if(this->p_ocr == 0){
        __android_log_print(ANDROID_LOG_DEBUG, "gariaman", "loadModel p_ocr");
        this->p_ocr = new OCR();
        this->p_ocr->init(mgr,"_ocr_224.param", "_ocr_224.bin", 224, use_gpu);
    }
}
void GARIAMAN::update(cv::Mat& image) {
    if (p_detector != 0 || p_ocr != 0){
        std::vector<Object> objects;
        p_detector->detect(image, objects, 0.5f, 0.2f);
        for (Object o : objects) {
            ...
            {
                char c[10];
                sprintf(c, "CAR:%.2d%%", (int)(o.confidence * 100));
                std::string text = c;
